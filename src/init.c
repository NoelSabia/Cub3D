...
/*   Updated: 2024/08/09 11:54:46 by oemelyan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void game_loop(void *mlx_copy)
{
    ...
    raycasting(mlx);
    // mlx->ray->minimap = mlx_new_image(mlx->mlx_p, SCREEN_WIDTH, SCREEN_HEIGHT);
...
void	init(t_mlx *mlx)
{
    printf("map size: [%d][%d]\n", mlx->parse->cols, mlx->parse->rows); //cols - actually rows
